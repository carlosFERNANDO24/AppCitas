...
export default function InicioPaciente() {
  ...
  const theme = {
    ...
    headerBgColor: darkMode ? '#1e1e1e' : '#fff',
    cardColor: darkMode ? '#1e1e1e' : '#fff',
    textColor: darkMode ? '#fff' : '#2C3E50',
    subtitleColor: darkMode ? '#aaa' : '#7F8C8D',
  };

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.backgroundColor }]}>
      <View style={[styles.header, { backgroundColor: theme.headerBgColor }]}>
        <Text style={[styles.greeting, { color: theme.textColor }]}>¡Hola, {userName}!</Text>
        <Text style={[styles.subtitle, { color: theme.subtitleColor }]}>Panel de Paciente</Text>
      </View>
      ...
      {loading ? (
        ...
        <>
          ...
          {proximaCita ? (
            <View style={[styles.appointmentCard, { backgroundColor: theme.cardColor }]}>
              <View style={styles.appointmentHeader}>
                ...
                <Text style={styles.appointmentDate}>{formatFechaHora(proximaCita.fecha_hora)}</Text>
              </View>
              <Text style={[styles.appointmentDetail, { color: theme.textColor }]}>
                <Text style={styles.label}>Médico:</Text> {proximaCita.medico?.nombre} {proximaCita.medico?.apellido}
              </Text>
              <Text style={[styles.appointmentDetail, { color: theme.textColor }]}>
            ...
            <View style={[styles.noDataCard, { backgroundColor: theme.cardColor }]}>
              <Ionicons name="calendar-outline" size={50} color={theme.subtitleColor} />
              <Text style={[styles.noDataText, { color: theme.subtitleColor }]}>No tienes citas programadas.</Text>
            </View>
          )}
          ...
          <Text style={[styles.sectionTitle, { color: theme.textColor }]}>Menú Principal</Text>
...
const styles = StyleSheet.create({
  ...
  header: { padding: 20, paddingTop: 50, borderBottomLeftRadius: 30, borderBottomRightRadius: 30, marginBottom: 20, elevation: 5 },
  ...
  appointmentCard: { borderRadius: 15, marginHorizontal: 20, padding: 20, marginBottom: 20, elevation: 3 },
  appointmentHeader: { flexDirection: "row", alignItems: "center", marginBottom: 10 },
  appointmentDate: { fontSize: 16, fontWeight: "bold", color: "#007AFF", marginLeft: 10, flexShrink: 1 },
  appointmentDetail: { fontSize: 14, marginBottom: 5 },
  label: { fontWeight: "bold" },
  appointmentButton: { backgroundColor: "#007AFF", borderRadius: 20, paddingHorizontal: 15, paddingVertical: 8, alignSelf: "flex-start", marginTop: 10 },
  appointmentButtonText: { color: "#fff", fontSize: 14, fontWeight: "bold" },
  noDataCard: { borderRadius: 15, marginHorizontal: 20, padding: 20, alignItems: "center", justifyContent: "center", elevation: 3, marginBottom: 20 },
  noDataText: { marginTop: 10, fontSize: 16, textAlign: "center" },
  statsGrid: { flexDirection: "row", justifyContent: "space-between", gap: 10, marginHorizontal: 20, marginBottom: 20 },
  statCard: { flex: 1, borderRadius: 15, padding: 15, alignItems: "center" },
  statNumber: { fontSize: 28, fontWeight: "bold", color: "#fff", marginTop: 5 },
  statText: { fontSize: 12, color: "#fff", textAlign: "center", marginTop: 2 },
  menuGrid: { flexDirection: "row", flexWrap: "wrap", justifyContent: "space-between", marginHorizontal: 20, marginBottom: 30 },
